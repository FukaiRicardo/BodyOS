...
exports.adaptProtocol = adaptProtocol;
exports.generateClientFeedback = generateClientFeedback;
...
async function callGroq(prompt, language, retries = 2) {
    if (!GROQ_API_KEY)
 ...
 */
function calculateHydrationScore(water_ml = 0) {
    if (water_ml < 500)
        return 0;
    if (water_ml < 1000)
        return 20;
...
function sanitizeNutritionPlan(plan, location) {
    if (!plan?.meals)
        return plan;
...
async function adaptProtocol(userData) {
    const lang = userData.language || 'pt';
    const locationContext = buildLocationContext(userData.location);
    const prompt = `
...
async function generateClientFeedback(data) {
